...
def get_authenticated_service(filename):
    flow = flow_from_clientsecrets(CLIENT_SECRETS_FILE, scope=YOUTUBE_UPLOAD_SCOPE, message=MISSING_CLIENT_SECRETS_MESSAGE)

    filename  = str(len(os.listdir('auth_file'))+1)+'.json'
    storage = Storage("auth_file/%s" % filename)
...
def resumable_upload(insert_request):
    ...
    while response is None:
        try:
            ...
            if response is not None:
                if 'id' in response:
                    return "Your youtube URL is https://www.youtube.com/watch?v=%s" % response['id']
                else:
    ...
    return "Error on video uploading"